...
<div class="py-8">
    <div class="max-w-5xl mx-auto">
        ...
        <form action="{{ route('blog-posts.store') }}" method="POST" class="bg-white rounded-xl shadow-lg p-8" id="blog-form">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea name="excerpt" rows="3"
                          class="w-full px-4 py-3 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-primary-accent focus:border-transparent">{{ old('excerpt') }}</textarea>
                ...
                <div class="flex flex-wrap gap-2" id="tags-container">
                    @foreach($tags as $tag)
                        <label class="tag-item">
                            <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="w-4 h-4 rounded border-neutral-300 text-primary-accent focus:ring-primary-accent">
                            <span>{{ $tag->name }}</span>
                        </label>
                    @endforeach
                </div>
